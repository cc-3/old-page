...
__ALL__ = ['lab0_git', 'lab0_git_expected']


lab0_git_expected = ['hello.sh', 'ex3.txt', 'ex4.txt']


# checks exercise 3
def check_ex3(filename):
    lookup = utils.parse_form(filename)
    ...
    if len(msg) == 0:
        ...
        return (grade, utils.incomplete('missing: ' + ','.join(msg)))
    else:
        return (grade, utils.failed('all answers are wrong'))


# checks exercise 4
def check_ex4(filename):
    f = open(filename, 'r')
    lines = f.read().strip()
    if len(lines) >= 0:
        line = lines.split(',')
...
# lab 0 autograder
def lab0_git(tmp, token):
    # hello.sh exercise
    ex2_result = ('2. git and Remotes', 20, utils.passed())
    ex3_result = ('3. Binary Alphabet',) + check_ex3(utils.join(tmp, 'ex3.txt'))
    ex4_result = ('4. 1000 $1 Bills',) + check_ex4(utils.join(tmp, 'ex4.txt'))
    grade = 0
    grade += ex2_result[1]
    grade += ex3_result[1]
    grade += ex4_result[1]
    table = [ex2_result, ex3_result, ex4_result]
    return (grade, table)
